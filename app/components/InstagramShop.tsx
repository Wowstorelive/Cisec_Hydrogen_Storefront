...
interface InstagramManualUploadBlockSettings {
  image?: { url: string; altText?: string; aspect_ratio?: number };
  local_video?: { sources: { format: string; url: string }[]; preview_image: { src: string } };
  more_info?: string;
  instagram_post_name?: string;
  link?: string;
...
interface InstagramShopBlock {
  id: string;
  type: 'instagram_manual_upload';
  settings: InstagramManualUploadBlockSettings;
}

interface InstagramShopProps {
  settings: InstagramShopSettings;
  blocks: InstagramShopBlock[];
...
// Placeholder for ProductItem component (simplified for popup)
function ProductItem({ card_product, template_enable_rate, template_enable_price, custom_widths, type, sizes }: any) {
  return (
    <div className="product-item flex gap-2 items-center">
      {card_product?.images?.nodes?.[0]?.url && (
        <img src={card_product.images.nodes[0].url} alt={card_product.title} className="w-16 h-16 object-cover rounded" />
      )}
      <div>
        <h4 className="text-sm font-medium">{card_product?.title}</h4>
        {template_enable_price && <p className="text-xs">€{card_product?.priceRange?.minVariantPrice?.amount}</p>}
      </div>
    </div>
  );
}

// Placeholder for LookbookItem component
function LookbookItem({ position_top, position_left, hotspot_style, children }: any) {
  return (
    <div
      className="z-3 absolute pointer"
      style={{
        top: `${position_top}%`,
        left: `${position_left}%`,
    ...
    >
      <span className="lookbook-animation heading-color w-44 h-44 inline-flex content-center relative">
        <span className="rounded-50 bg-white w-30 h-30 inline-flex content-center">
          {hotspot_style === 'plus' ? (
            <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
              <path fillRule="evenodd" clipRule="evenodd" d="M5 0C4.44772 0 4 0.447715 4 1V4L1 4C0.447715 4 0 4.44771 0 5C0 5.55228 0.447715 6 1 6H4V9C4 9.55229 4.44772 10 5 10C5.55228 10 6 9.55228 6 9V6H9C9.55228 6 10 5.55229 10 5C10 4.44772 9.55228 4 9 4L6 4V1C6 0.447715 5.55228 0 5 0Z" fill="currentColor"></path>
            </svg>
          ) : (
            <span className="w-2 h-2 rounded-full bg-current"></span> // Simple dot
          )}
        </span>
      </span>
      {children}
...
export function InstagramShop({ settings, blocks, allProducts }: InstagramShopProps) {
  const { 
    section_width, 
    color_scheme, 
    reset_spacing, 
        ...
        {blocks.length > 0 && (
          ...
          >
            {blocks.map((block, index) => {
              const blockSettings = block.settings as InstagramManualUploadBlockSettings;
              const motionDelay = index * 50;
              const animationOrder = index + 1;

              const image = blockSettings.image;
              const localVideo = blockSettings.local_video;
              const imageAlt = image?.altText || 'Instagram Shop';
              const aspectRatio = getAspectRatio(image, localVideo);

              const products = [];
              for (let i = 1; i <= 5; i++) {
                const productIdKey = `product__${i}` as keyof InstagramManualUploadBlockSettings;
              ...
              return (
                <SwiperSlide key={block.id}>
                  <div
                    className={clsx(
                      'instagram-item instagram-item__media--ratio swiper-slide pointer',
                  ...
                  >
                    ...
                    {products.map((product, productIndex) => {
                      const offsetTopKey = `offset_top__${productIndex + 1}` as keyof InstagramManualUploadBlockSettings;
                      const offsetLeftKey = `offset_left__${productIndex + 1}` as keyof InstagramManualUploadBlockSettings;
                    ...
                    {showPopup?.blockId === block.id && (
                      <div className="instagram-item__media--ratio popup-content popup-index-{{ forloop.index }} absolute inset-0 bg-white shadow-lg flex">
                          ...
                          <div className="absolute-sm inset-0 overflow-y-auto custom-scrollbar px-15 px-md-30 py-30">
                            ...
                            {blockSettings.link && (
                              <Link
                                ...
                                aria-label={blockSettings.instagram_post_name || 'View Post'}
                              ...
                              >
                                ...
                                {blockSettings.instagram_post_name || 'View Post'}
                              </Link>
                            )}
                          </div>
                        </div>
